    ...
    <section id="community" className="py-32 bg-white overflow-hidden">
      <div ref={revealRef} className="container mx-auto px-6 reveal">
        <div className="text-center mb-20">
          <h2 className="text-emerald-600 font-bold uppercase tracking-widest mb-4">Community</h2>
          <h3 className="text-4xl md:text-5xl font-black text-stone-900 tracking-tight">Trusted by Local Farmers</h3>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 stagger-reveal active">
          {reviews.map((rev, i) => (
            <div key={i} className="bg-stone-50 p-10 rounded-[2.5rem] border border-stone-100 flex flex-col h-full hover:bg-white hover:shadow-[0_20px_50px_rgba(0,0,0,0.06)] transition-all duration-500 group">
              <div className="flex gap-1 mb-8">
                {[...Array(5)].map((_, j) => <Star key={j} className="w-5 h-5 fill-amber-400 text-amber-400" />)}
              </div>
              <p className="text-stone-600 leading-relaxed mb-10 flex-grow text-lg italic font-medium group-hover:text-stone-900 transition-colors">"{rev.text}"</p>
              <div className="flex items-center gap-4">
                <img src={rev.img} alt={rev.name} className="w-14 h-14 rounded-2xl object-cover shadow-lg group-hover:scale-105 transition-transform" />
                <div>
                  <h4 className="font-black text-stone-900 text-lg">{rev.name}</h4>
                  <p className="text-xs text-stone-400 font-bold uppercase tracking-widest">{rev.role}</p>
                </div>
              </div>
            </div>
          ))}
